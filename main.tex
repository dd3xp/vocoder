...
\textbf{研究目标}: 给定 MIDI 乐谱, 预测真实演奏对应的逐声部帧级基频曲线 $f_0(t)$ 与振幅曲线 $\mathrm{amp}(t)$, 帧率 100 Hz。 模型应当具备如下三方面能力: 第一, 能从离散稀疏的音符序列中读出连续稠密的演奏控制意图, 而非简单地把音符 onset/offset 复刻到帧级网格; 第二, 能同时利用过去与未来的乐谱上下文, 覆盖单音局部、乐句中段、乐句首尾三种时间尺度上的演奏惯例; 第三, 用一套统一的架构同时适配单声部与多声部输入规模, 并预留沿声部维作用的可选交互通道, 以便检验同时发声的其他声部是否真正提升预测能力。

\textbf{研究内容}: (1) 设计可量化音乐时序语境的 20 维输入特征空间, 覆盖单音层、局部旋律层、乐句结构层三个时间尺度, 并以乐器族独热向量作为跨层统领, 让模型在编码阶段同时获得显式的音乐先验与隐式的统计先验。 (2) 构建一套统一骨干, 用双向 Mamba 与双向 GRU 串联捕获长程时序依赖, 中间预留沿声部维作用的跨声部交互模块, 在需要时能依据同时发声的其他声部对本声部表示作再加权; $S = 1$ 时该模块退化为恒等映射, 整套架构在单声部与多声部输入下统一工作。 (3) 在 URMP\upcite{urmp}、Bach10\upcite{bach10}、PHENICX-Anechoic 与 TRIOS 四个公开数据集所覆盖的真实演奏数据上, 评估帧级 $f_0$ 的原始音高准确率与帧级振幅的 Pearson 相关性, 并用多随机种子配对检验衡量跨声部交互的真实增益。 (4) 分析模型在不同乐器族上的预测能力差异, 把预测残差与各乐器的发声物理 (颤音、气压、弓压) 相关联, 为后续音色合成在乐器与织体层面上的差异化建模提供依据。
...
本文使用 URMP\upcite{urmp}、Bach10\upcite{bach10}、PHENICX-Anechoic 与 TRIOS 四个公开数据集。 URMP 由罗切斯特大学发布, 包含 44 段古典室内乐, 共 13 种乐器, 提供高质量分轨录音、MIDI 乐谱与帧级 $f_0$ 标注; Bach10 由西北大学发布, 包含 10 段四声部巴赫圣咏, 每段都有 4 件乐器的独立录音; PHENICX-Anechoic 提供无混响条件下的多声部古典片段, 含弦乐与管乐多种编制; TRIOS 是更新的三重奏数据集, 进一步补充小编制场景。 四个数据集合并覆盖 13 种古典常用乐器 (弦乐 vn、va、vc、db; 木管 fl、ob、cl、sax、bn; 铜管 tpt、tbn、hn、tba), 总帧率 100 Hz (帧间隔 10 ms)。 本文同时考虑两种评测口径: \textbf{单声部口径} 把每条声部独立分轨当作独立样本, 合并 4 个数据集后按曲名 80:20 划分得 33 条测试声部轨, 是本文主结果的默认评测设置; \textbf{多声部口径} 把同一曲目的所有声部并行进入模型, 由于 PHENICX 中存在同一声部内多件乐器加倍演奏的情况无法按声部隔离, 故多声部实验仅用 URMP、Bach10、TRIOS 三个数据集, 合并 58 首曲目, 按曲名 80:20 划分得 12 首测试曲、共 43 条测试声部轨, 用于检验跨声部交互模块的真实增益, 详见 §V.1.7。

\subsubsection{数据勘探（预处理）}

每条音轨经如下流水线: 从原始 wav 中按 10 ms 帧间隔重采样得到帧网格; 用 CREPE\upcite{crepe} 提取帧级 $f_0$ 并以人工核对的 ground-truth 校正; 用 STFT 短时窗口计算帧级 RMS, 进一步取对数域得到 $\log\mathrm{amp}$; 从同步 MIDI 文件解析每个音符的 onset、offset、pitch、velocity, 并按 100 Hz 网格生成与音频对齐的帧级音符序列。 划分严格按曲名以 80:20 进行, 同一首曲子的所有声部要么全部在训练集、要么全部在测试集, 既避免同一作品的不同乐器版本跨集出现, 也杜绝跨声部信息泄漏。
...
进一步地, 一个声部该多响理论上可能受到同时发声的其他声部的影响 (例如主旋律放开、伴奏收敛)。 为检验这一可能性, 本文在 Mamba 编码器与 GRU 之间预留一段沿声部维作用的跨声部交互模块, 让每个声部表示在需要时能依据其他声部被重加权; 当只有一个声部输入 ($S = 1$) 时该模块自然退化为恒等映射, 整套架构在单声部与多声部两种口径下统一工作。 整体张量流见图~\ref{fig:arch}。
...
\end{equation}
形式的特征级调制, 输入声部数 $S = 1$ 时该模块自然退化为恒等映射, 整套架构无需修改即可处理任意声部数; 跨时序回归再经双向 GRU (单向隐藏维 256, 2 层, 双向拼接输出 512 维) 做最后一段时序整合。 双头解码部分中: $f_0$ 头输出 81 维音高 logits $\mathbf{z}_t$, 但其预测目标并非 MIDI 上已经标注的绝对半音, 而是音符内部及音符过渡处的音分级偏移 ($\pm 200$ 音分以内, 步长 5 音分)、滑奏与颤音的频率轨迹、以及乐句末尾弱信号段的稳定性, MIDI 绝对音高直接由特征注入并不计入 81 类预测; 推理时对 logits 取 softmax, 以软取偏移叠加音符基准音高还原 Hz,
...
\STATE $\mathbf{h} \leftarrow \textit{BiMamba}(\textit{LinearProj}(\text{feat}))$ \COMMENT{$S \times T \times 128$, 权重共享, 6 层}
\STATE $\mathbf{h} \leftarrow \textit{CrossVoiceInteraction}(\mathbf{h})$ \COMMENT{沿声部维 $S$ 交互; $S{=}1$ 时恒等}
\STATE $\mathbf{h} \leftarrow \textit{BiGRU}(\mathbf{h})$ \COMMENT{$S \times T \times 512$ (2 层, 单向隐 256, 双向拼接)}
...
本文在两套评测口径下分别报告主要结果, 二者使用同一套架构与同一套训练流程 (300 个 epoch, 4 个随机种子, 多种子均值 $\pm$ 标准差汇报)。

\textbf{单声部口径} (4 个数据集合并, 33 条测试声部轨, 跨声部交互模块取恒等映射): 模型达到帧级 $f_0$ 原始音高准确率 RPA $\approx \mathbf{94.6\%}$, 中位音分误差约 11.7 cents, 线性域 $\mathrm{amp}$ Pearson 相关 $\mathbf{0.671 \pm 0.005}$。

\textbf{多声部口径} (3 个数据集合并, 43 条测试声部轨, 跨声部交互模块取 \texttt{deep} 多深度交替变体): RPA $\approx \mathbf{92.5\%}$, $\mathrm{amp}$ Pearson $\mathbf{0.615 \pm 0.041}$。

两套口径的整体均值差异主要来自测试集乐器分布: 多声部 43 条声部轨中弦乐 21、木管 12、铜管 10, 弦乐占比 (49\%) 高于单声部测试集 (15/33 = 45\%), 而弦乐又是可预测度最低的族 (详见 §V.1.5、§V.1.7), 整体均值被相应拉低。 两套口径在乐器内可预测度数值上保持一致, 跨声部交互的真实增益、稳定迁移与统一模型证据见 §V.1.7。

下文对单声部主结果做共性分析: $f_0$ 的 RPA 接近 100\% 的量纲上限, 说明基频在 MIDI 半音约束之下基本可被完整预测; 偶发的 5\% 余差大多落在乐句末尾的弱信号段与两个半音之间的滑奏过渡, 这两类位置上 $f_0$ 标注本身也不稳定, 因此不构成模型的真实瓶颈。 振幅的 Pearson 在 0.67 一线, 表明可预测部分主要由乐句轮廓与音符内动态包络支撑, 这两类信号都强烈依赖于 MIDI 中可直接读取的乐句结构与音符位置; 残差中较高频段的微观动态 (例如颤音引起的振幅调制 AM、乐句中段的力度微抖、弓压与气流的连续起伏) 则受真实演奏的不可观测因素影响, 不可由 MIDI 单独决定。 该残差可能来自三种成分: (a) 跨架构都未能越过的经验饱和点, (b) 默认指标自身放大特定噪声成分的产物, (c) 人耳真正可感知的演奏表达流失, §V.1.3 配置扫描、Oracle 负结果、集成误差相关与多指标重构四项分析依次定位, 再由主观听感实验作收束。

\subsubsection{经验饱和点的稳健性}

本节四项分析 (配置扫描、Oracle 负结果、集成误差相关与多指标重构) 均在前述\textbf{单声部口径}下进行 (33 条测试轨), 用以刻画振幅 Pearson 在仅乐谱设置下的经验饱和带; 与多声部口径的对照见 §V.1.7。

\paragraph{配置扫描}\quad
为判断振幅 Pearson 的饱和是否仅为单一模型的不足, 本文沿四条干预轴对该任务做了系统配置扫描, 共覆盖 16 个方向类、累计 200 余个有记录的训练配置: 第一条轴是架构与容量 (双向 GRU、Transformer、S4D、Mamba, 参数量 1 M 至 30 M); 第二条轴是损失与训练目标 (相关性损失、排序损失、多分辨率谱风格损失、力度感知教师蒸馏、异方差/瞬态加权损失、随机权重平均、平滑目标训练); 第三条轴是目标与特征变换 (高斯平滑、秩、CDF 均衡、巴特沃斯低通等目标表示, 小波/经验模态分解/陷波/可学习滤波等去噪范式, 逐曲归一化、ADSR 先验, 合成与分布外语料扩充); 第四条轴是集成与 oracle (异构多架构集成、乐谱力度注入、真值 $f_0$ oracle)。 每个方向取该方向最优配置后 (图~\ref{fig:closure}), 全部 16 个最优振幅 Pearson 均落在 $r \in [0.60, 0.69]$ 的窄带内, 其中 13/16 落在单模型基线 $\pm 0.02$ 以内, 没有任何一条方向单独突破 0.70。 这一致性显示, 在本文数据、特征与默认评测口径下, 振幅 Pearson 在 $\approx 0.69$ 一线呈现稳定饱和, 是当前设置下跨架构、跨损失、跨特征变换共享的可预测性瓶颈。 该饱和是本文所选数据与特征空间下的经验现象, 不直接外推为"任务理论上限"。
...
\paragraph{Oracle 负结果}\quad
配置扫描确立了饱和点跨架构稳健, 但并未排除"模型缺失某一关键输入"的可能。 本文为此对两个最有力的候选缺失特征直接做 oracle 检验。 (i) 音高耦合 oracle: 测试时把模型预测的 $f_0$ 偏差替换为真值 $f_0$ 导出的偏差, 这一干预把 $f_0$ RPA 从 94.6\% 提至 98.0\%, 颤音深度误差大致减半, 但振幅 Pearson 仅比同种子基线高 $\Delta r = +0.002$, 完全落在 $\pm 0.005$ 的种子噪声以内, 说明音高耦合信息不是缺失的振幅信号。 (ii) 乐谱力度 oracle: 从 URMP、PHENICX、TRIOS 的源 MIDI 重抽含有与录音无关的非平凡力度标记的力度轨, 作为第 21 维输入加入特征向量, 同样四种子训练得振幅 Pearson $0.666 \pm 0.008$, 与 20 维基线统计上分不开 ($\Delta r = -0.005$, 处于合并种子噪声内)。 两个 oracle 都未抬动饱和点, 这说明残差更可能来自演奏者层面的响度意图 (GERM 模型\upcite{germ} 中的表达与运动分量), 该意图在 MIDI 乐谱中没有与乐器无关的编码, 因此本来就在仅乐谱口径之外; 当然, 本文仅检验了 $f_0$ 与 MIDI 力度两个最有力的候选缺失特征, 并不构成对所有可能输入的穷尽排除。

\paragraph{集成误差相关}\quad
若饱和点确为跨架构共享, 它应在异构集成的误差相关上留下可量化的痕迹。 这里借助 Perrone 与 Cooper 的集成方差缩减框架\upcite{perronecooper}: 对 $N$ 个预测器取平均, 记单成员均方误差均值为 $\bar{E}$、成员间误差两两相关为 $\rho_e$, 集成均方误差为
...
默认指标约 0.69 与重构指标约 0.90 之间究竟丢失了什么? 本文用一项 $N = 26$ 的强制二选一 A/B 听感实验作答。 实验材料选取了振幅可预测度最低的乐器族 (弦乐, 见 §V.1.5 表~\ref{tab:per-family}) 中的 20 秒小提琴片段, 用同一 8 谐波加性合成器分别用真实 $(f_0, \mathrm{amp})$ 与模型预测的 $(f_0, \mathrm{amp})$ 驱动, 让被试在哪一版噪声更多、哪一版表达更多两个维度上做选择; 在最难的乐器族上验证听感结论, 等价于对所有乐器给出相对保守的下界。 结果见表~\ref{tab:listening}: 噪声维度 26 名被试全部选择真实版 ($p \approx 3 \times 10^{-8}$); 表达维度则接近随机 (15:11, $p \approx 0.56$)。 在该样例与该合成器设置下, 被试并未稳定区分真实与预测版本的表达差异, 这一结果初步说明默认 Pearson 未捕获的部分未必全部对应可听感的演奏表达损失; 但样本量与片段长度均较小, 是否能外推到所有乐器、所有曲风、所有下游合成器, 仍需更大规模、更多片段的主观研究进一步验证。
...
把 33 条单声部测试轨按乐器族划分: 铜管 ($n = 9$, tpt、tbn)、木管 ($n = 9$, fl、ob、cl、sax、bn)、弦乐 ($n = 15$, vn、va、vc), 9 成员异构集成在默认线性 RMS 指标下的逐族振幅 Pearson 见表~\ref{tab:per-family}: 铜管 0.857、木管 0.702、弦乐 0.575, 三族的 95\% 自助置信区间互无重叠, 族间差距稳健, 不是小样本假象。 需说明的是, §IV.1.1 列出的 13 种乐器是四个数据集合并后的总覆盖范围, 按曲名 80:20 划分后, db、hn、tba 三种乐器恰好被划入训练集而未进入这 33 条单声部测试轨, 因此本节族表不列这三种; 多声部口径下 (43 条声部轨) 由于划分粒度从声部轨改为曲目, 这三种乐器进入了测试集, 详见 §V.1.7 表~\ref{tab:per-family-poly}。 这一排序具有清晰的物理解释: 弦乐 $\mathrm{amp}$ 曲线中含有较高频段的颤音调幅成分\upcite{mellody}, 这一成分由演奏者的弓速变化与左手揉弦控制, 在 MIDI 乐谱中没有对应记号, 因而不可由 MIDI 单独推断; 木管的振幅则由气流压力与音孔切换共同塑形, 其中音孔切换由乐谱决定可被预测, 气流压力的连续起伏则需要演奏者意图作为额外输入; 铜管的振幅主要由气压包络与音符结构决定, 与 MIDI 的关联度最高, 因此可预测度最强。 由于弦乐处在最低端, 主观听感实验也正是为此特意选取弦乐 (小提琴) 作答, 以验证最难的乐器族上模型预测仍能承受听感校验。 长远来看, 给弦乐单独引入颤音生成模块, 或者把演奏者意图作为额外输入, 可能是进一步提升弦乐振幅可预测度的可行路径。

\begin{table}[htbp]\centering
\caption{单声部口径下乐器族振幅 Pearson (33 条测试轨, 9 成员异构集成)}\label{tab:per-family}
\begin{tabular}{lcc}
\toprule
乐器族 & 测试声部数 & $\mathrm{amp}$ Pearson \\
\midrule
铜管 (tpt、tbn) & 9 & 0.857 \\
木管 (fl、ob、cl、sax、bn) & 9 & 0.702 \\
弦乐 (vn、va、vc) & 15 & 0.575 \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{方差频带分解}

逐乐器族分析已经把残差落到具体乐器, 进一步本文沿频率方向定位残差所占的物理通道。 用 4 阶巴特沃斯滤波器\upcite{butterworth} 把测试集对数振幅目标分到四个有物理动机的频带: 子乐句带 ($<0.5$ Hz, 含逐轨录音增益与乐句级动态)、音符包络带 ($0.5$\,--\,$3$ Hz)、颤音调幅带 ($3$\,--\,$10$ Hz)、高频带 ($10$\,--\,$48$ Hz, 弓/气/话筒噪声)。 测试轨上真值对数振幅的方差占比依次约为 48\%、30\%、10\%、2\% (余下约 10\% 为有限滤波过渡带的频带间泄漏), 9 成员异构集成对真值的逐带 Pearson 为子乐句带 0.895、音符带 0.759、颤音带 0.631、高频带 0.230 (图~\ref{fig:varbands})。 子乐句带方差占比最高且被捕捉得最好, "模型输在慢分量"的流行直觉被实测否定; 颤音带尽管只占真值方差约 10\%, 却承载了约 15\% 的总残差均方误差。 定义某频带的过表征比为其残差份额与方差份额之比,
\begin{equation}\label{eq:overrep}
\rho_{\text{band}} = \frac{\mathrm{MSE}_{\text{band}} \big/ \textstyle\sum_b \mathrm{MSE}_b}{\sigma^2_{\text{band}} \big/ \textstyle\sum_b \sigma^2_b},
\end{equation}
按未取整的频带份额计, 颤音带 $\rho_{\text{band}} \approx 1.4$, 指向 $3$\,--\,$10$ Hz 是主导振幅 Pearson 经验饱和的频带。 作为稳健性检验, 把固定的巴特沃斯频带换成数据自适应的变分模态分解\upcite{vmd} (令 $K = 4$ 个模态的中心频率从信号中自行发现), 发现的模态 (平均 0.00、1.86、5.52、20.67 Hz) 与四个手选频带重合, 逐带 Pearson (0.85、0.70、0.48、0.17) 定性一致, 说明四带分解不是所选滤波截止的人为产物。 颤音带恰好对应弓速变化与左手揉弦控制\upcite{mellody}, 在 MIDI 乐谱中没有对应记号, 与逐乐器族中弦乐显著落后于铜管的实测排序完全对得上。

\begin{figure}[H]\centering
\includegraphics[width=0.80\linewidth]{figures/variance_bands.pdf}
\caption{测试集对数振幅四个频带的方差占比 (柱, 左轴) 与 9 成员异构集成对真值的逐带 Pearson (线, 右轴)。 子乐句带方差占比最高且预测最好, 颤音带方差占比虽小却主导残差, 高频带最难预测。}
\label{fig:varbands}
\end{figure}

\subsubsection{多声部扩展}

\paragraph{跨声部交互的动机与结构}\quad
前述单声部实验把每条声部独立预测, 完全不利用其他声部信息。 实际演奏中, 一个声部的响度分配理论上可能受到同时发声的其他声部的影响 (例如主旋律放开、伴奏收敛)。 本文据此检验在统一骨干上加入跨声部交互模块的真实增益, 设四个变体作严格对照: \texttt{none} 用恒等映射替代该模块, 让各声部独立运行 (零交互对照基线); \texttt{attn} 沿声部维做一次多头自注意力; \texttt{deep} 把时序卷积与声部维注意力交替堆叠三层, 在多个深度上反复交互; \texttt{film} 由其他声部聚合得到的上下文产生逐通道仿射系数 (式~\ref{eq:film}), 以特征级仿射调制本声部表示。 四个变体共用同一骨干, 任何差异只能归因于交互机制本身。 多声部实验使用 URMP、Bach10、TRIOS 三个数据集 (58 首曲目, 43 条测试声部轨), 训练 300 个 epoch、4 个随机种子。

\paragraph{跨声部交互的消融}\quad
§V.1.2 的多声部主结果 ($0.615 \pm 0.041$) 取的是 \texttt{deep} 多深度交替变体。 在 4 个随机种子的配对检验下 (表~\ref{tab:interaction}), 无交互基线 $\mathrm{amp}$ Pearson 为 $0.580 \pm 0.035$, 加入 \texttt{deep} 交互后升至 $0.615 \pm 0.041$, 增益 $+0.035$, 配对 $t \approx 1.10$, 在当前数据规模下尚未达到统计显著。 这一接近零的增益与单声部经验饱和点的结论彼此呼应: 跨声部协同所需的额外信息, MIDI 乐谱同样不足以承载, 即便显式引入交互机制, 也只能换来配对检验未达显著的小幅改进。 跨声部交互并不会破坏整体性能, 整体走势依然贴近统一骨干的预测能力, 可以作为下游音色合成的稳定输入。

\begin{table}[htbp]\centering
\caption{跨声部交互模块的消融结果 (43 条声部轨, 4 种子, 振幅 Pearson)}\label{tab:interaction}
\begin{tabular}{lcc}
\toprule
变体 & 均值 $\pm$ 标准差 & $t$ 值 \\
\midrule
...
\paragraph{多声部逐乐器族分布}\quad
多声部 43 条测试声部轨上的逐乐器族 Pearson (表~\ref{tab:per-family-poly}) 仍呈"铜管 > 木管 > 弦乐"的稳定排序 ($0.825 / 0.603 / 0.522$), 与单声部口径下的 $0.857 / 0.702 / 0.575$ 完全同向; 进一步逐乐器汇总 (表~\ref{tab:per-inst}) 显示同族内部各乐器之间的相对位次也保持稳定, 弦乐内部的 vn、va、vc、db 互相邻近, 铜管内部的 tbn、hn、tpt、tba 互相邻近。 跨两种评测口径的族序与乐器内可预测度数值一致, 是本文整体 Pearson 数值最关键的支撑事实, 直接决定了 §V.1.8 关于结论适用范围的判断。

\begin{table}[htbp]\centering
\caption{多声部口径下乐器族振幅 Pearson (43 条声部轨, 4 种子)}\label{tab:per-family-poly}
\begin{tabular}{lcc}
\toprule
乐器族 & 测试声部数 & $\mathrm{amp}$ Pearson \\
\midrule
...
\begin{table}[htbp]\centering
\caption{多声部口径下逐乐器振幅 Pearson (4 种子, 按均值降序)}\label{tab:per-inst}
\begin{tabular}{lcccc}
\toprule
乐器 & 中文名 & 乐器族 & 测试声部数 & $\mathrm{amp}$ Pearson \\
\midrule
tbn & 长号 & 铜管 & 3 & $0.867 \pm 0.025$ \\
hn & 圆号 & 铜管 & 2 & $0.823 \pm 0.029$ \\
tpt & 小号 & 铜管 & 4 & $0.801 \pm 0.062$ \\
tba & 大号 & 铜管 & 1 & $0.800 \pm 0.115$ \\
cl & 单簧管 & 木管 & 5 & $0.670 \pm 0.057$ \\
sax & 萨克斯 & 木管 & 4 & $0.599 \pm 0.065$ \\
va & 中提琴 & 弦乐 & 4 & $0.581 \pm 0.118$ \\
vc & 大提琴 & 弦乐 & 3 & $0.561 \pm 0.069$ \\
db & 低音提琴 & 弦乐 & 2 & $0.499 \pm 0.058$ \\
vn & 小提琴 & 弦乐 & 12 & $0.497 \pm 0.114$ \\
bn & 巴松 & 木管 & 3 & $0.497 \pm 0.049$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{稳定迁移与模型统一}\quad
架构上, 本文模型在 $S = 1$ 时跨声部模块退化为恒等映射, 因此同一套权重可无修改地处理独奏与合奏两种输入; 但架构本身只保证可运行, 不保证性能稳定, 后者仍需实验确认。 把只在合奏数据上训练 (从未见过单声部输入) 的多深度模型, 直接以单声部方式喂入第 V.1.2 节的 33 条单声部测试轨, 得振幅 Pearson 0.628、$f_0$ RPA 0.906、分族 0.845 (铜管) / 0.607 (木管) / 0.511 (弦乐); 与单声部专用模型基线 (0.671 / 0.946) 仅相差约 0.05, 0.628 也正好落在多深度模型自身的 4 种子区间 0.554\,--\,0.641 内。 也就是说, 跨两种评测模式 (本职合奏与单声部迁移) 的一致性, 处于该模型自身的种子噪声包络之内。 实验表明这种退化在本文测试集上并未造成灾难性性能下降, 可作为统一架构可行性的初步证据; 但本结论仅基于单一检查点与本文测试集, 在更大规模合奏数据或不同分布下的稳定性, 仍需进一步检验。

\subsubsection{适用范围说明}

需要明确, 本节所报告的整体数值 (单声部 RPA $\approx 94.6\%$、振幅 Pearson $0.671 \pm 0.005$; 多声部 RPA $\approx 92.5\%$、振幅 Pearson $0.615 \pm 0.041$) 都依赖于本文测试集的乐器分布: 单声部 33 条测试轨中弦乐 15、木管 9、铜管 9, 多声部 43 条测试声部轨中弦乐 21、木管 12、铜管 10, 两套口径下弦乐占比均较高, 因而整体均值被弦乐这一最难的族强烈拉低。 真正稳定的量是表~\ref{tab:per-inst} 中的逐乐器 Pearson, 它由各乐器自身的物理产声机制与 MIDI 之间的信息差距决定 (颤音、气压调控、弓速等不可观测因素), 不依赖于测试集中各乐器各占多少。 因此, 若将本文模型迁移到一个铜管比例更高的曲库, 整体 Pearson 会上移; 反过来, 在以弦乐独奏为主的曲库上整体 Pearson 会下移。 但每一种乐器自己的可预测度数值基本保持稳定, 主观听感实验也正是为此选取最难的小提琴作答, 以验证这种乐器内部的稳定饱和点确实可以承受听感校验。 该乐器内稳定性同样意味着, 后续工作引入更大规模、更高多样性的真实录音时, 主要会改变两表中的族间相对比例与整体均值, 而不会改变本文得到的乐器内可预测度排序及其物理解释。 此外, 本文数据规模 (58 首、约 88 分钟、主要为西方古典室内乐) 仍较小, 上述结论是否可外推到非西方音律体系、爵士摇摆等其他演奏风格, 仍需更大规模的真实录音进一步验证。
...
针对演奏信息模型这一关键环节, 设计了 20 维三层金字塔特征 + 双向 Mamba 与双向 GRU 串联骨干 + 跨声部交互模块 + $f_0$ 分类与 $\mathrm{amp}$ 音符感知回归双头的统一架构。 单声部口径下基于 URMP、Bach10、PHENICX-Anechoic 与 TRIOS 四个数据集评估 (33 条测试声部轨), 模型在 4 种子均值下达到 $f_0$ RPA 约 94.6\%、$\mathrm{amp}$ Pearson 约 $0.671 \pm 0.005$; 多声部口径下基于 URMP、Bach10 与 TRIOS 三个数据集评估 (43 条测试声部轨, 跨声部 \texttt{deep} 变体), 达到 $f_0$ RPA 约 92.5\%、$\mathrm{amp}$ Pearson 约 $0.615 \pm 0.041$。 模型参数仅 7.66 M, 可作为下游合成的稳定输入。 跨声部交互模块的消融显示加入交互带来 $+0.035$ 的边际增益, 多种子配对检验未达显著, 与振幅经验饱和点的结论一致。

\subsection{不足}

数据规模仍偏小: 单声部口径虽覆盖 13 种西方古典常用乐器, 但多声部口径实际用于跨声部交互检验的数据规模更有限, 且尚未覆盖爵士、电子、流行等其他演奏风格, 对这些风格的迁移能力未验证。 模型对长程音乐结构 (如乐章级反复) 的捕获仍依赖统计先验, 在转调或速度突变处的表现有限。 跨声部交互模块带来的增益虽与振幅经验饱和点的结论方向一致, 但 0.035 的差值仅由 4 个随机种子估得, 仍需更大规模数据加以确认。 与下游合成器的联合训练、以及向更大编制 (四重奏、室内乐) 的扩展, 留作未来工作。
...
\bibitem{butterworth} S. Butterworth, ``On the Theory of Filter Amplifiers,'' \emph{Wireless Engineer}, vol.~7, pp.~536--541, 1930.

\bibitem{vmd} K. Dragomiretskiy and D. Zosso, ``Variational Mode Decomposition,'' \emph{IEEE Trans. on Signal Processing}, vol.~62, no.~3, pp.~531--544, 2014.
